...
athelits= pd.read_csv("olympics dataset/athlete_events.csv")
region = pd.read_csv("olympics dataset/noc_regions.csv")
def data_preprocessor():
    global athelits,region
    df=pd.merge(athelits,region,on="NOC")
...
def duplicate_rows_remover(df1,df2):
    ...
    return df1,df2
def medals_tally_calculator(df):
    ...
    medal_pivot.sort_values(by=["Gold", "Silver", "Bronze"], ascending=False).head(30)
    return medal_pivot
...
#     ax.bar(medalscounts,color=["gold","silver","brown","green"])
#     plt.show
